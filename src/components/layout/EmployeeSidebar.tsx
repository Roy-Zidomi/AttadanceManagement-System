  ...
  const sidebarContent = (
    <>
      ...
      <div className="px-3 pt-2 pb-1">
        <p className="px-3 text-[10px] font-semibold uppercase tracking-widest text-slate-500">Menu</p>
      </div>
      ...
      <div className="mt-auto px-3 py-3 border-t border-slate-200/60 dark:border-white/5 space-y-1.5">
        <Link href="/employee/profile" onClick={() => setOpen(false)} className="hover:bg-slate-100 dark:hover:bg-white/5 transition-colors rounded-lg px-2 py-1.5 flex items-center gap-2.5 group">
          ...
          <div className="overflow-hidden">
            <p className="text-xs font-medium text-slate-700 dark:text-white truncate">{session?.user?.name || "Employee"}</p>
